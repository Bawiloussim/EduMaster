              ...
              return (
                <div key={q._id} className={`bg-white rounded-xl border-2 p-5 ${correct ? 'border-success/30' : correct === false ? 'border-danger/30' : 'border-gray-200'}`}>
                  <div className="flex items-start gap-3">
                    {correct ? <CheckCircle className="h-5 w-5 text-success shrink-0 mt-0.5" /> : correct === false ? <XCircle className="h-5 w-5 text-danger shrink-0 mt-0.5" /> : <div className="h-5 w-5 rounded-full border-2 border-gray-300 shrink-0 mt-0.5" />}
